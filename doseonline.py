...
async def on_message(message):
    ...
    if message.content.startswith("!등록") and not message.content == '!등록여부':
        if message.author.guild_permissions.administrator and (message.author.id == 695428161214611526):
            try:
    ...
    if message.content == "!임시퇴근":
        try:
            ...
            embed = discord.Embed(title='', description=f'**{message.author.mention}** 님이 임시퇴근 하셨습니다',
                                  color=discord.Colour.red())
            embed.set_author(name=message.author, icon_url=message.author.avatar_url)
            embed.set_footer(text='임시퇴근 시간: ' + time.strftime('%m-%d %H:%M') + '\n' + '근무시간: ' + str(worktime) + str(danwe))
            await client.get_channel(int(achannel)).send(embed=embed)
            await message.channel.send(f'{message.author.mention} 임시퇴근 완료')
        except Exception as e:
                embed = discord.Embed(title='❌  오류', description=f'오류가 발생하였습니다\n`{str(e)}`', color=0xFF0000)
                await message.channel.send(embed=embed)
